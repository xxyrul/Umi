...
export default function MasterListingScreen() {
  ...
  // Generate rich formatted listing message with clean title, location and loan estimate
  const getFormattedShareMessage = (item: PropertyListing) => {
    ...
    const numPrice = typeof item.harga === "number" ? item.harga : Number(String(item.harga || "0").replace(/[^0-9.]/g, ""));
    const mortgage = !isNaN(numPrice) && numPrice > 0 ? calculateMortgage(numPrice) : null;
    const monthlyEstimateStr = mortgage ? `~RM ${mortgage.monthlyInstallment.toLocaleString()} / bln` : "";

    const sizeStr = formatSizeLabel(item.keluasan) || "";
    const isBM = language === "BM";
    const cleanTitle = cleanListingTitle(item.tajuk) || "Hartanah Untuk Dijual";
    const locInfo = resolveListingLocation(item);
    ...
    if (isBM) {
      return (
        ...
        `• Bilik: ${item.bilikTidur || 0} Bilik Tidur, ${item.bilikAir || 0} Bilik Air\n` +
    ...
    return (
      ...
      `📍 Location: ${locText}\n\n` +
      `✨ Specifications:\n` +
      `• Beds/Baths: ${item.bilikTidur || 0} Beds, ${item.bilikAir || 0} Baths\n` +
  ...
  // 1-Tap Copy Full Copywriting Text to Clipboard
  const handleCopyListingText = async (item: PropertyListing) => {
    ...
    try {
      ...
      if (Platform.OS === "android" && NativeModules.MultiShare) {
        try {
          await NativeModules.MultiShare.shareMultipleImages(
            preparedUris,
            cleanListingTitle(item.tajuk) || "Property Listing",
            message
          );
          return;
      ...
      if (isAvailable) {
        await Sharing.shareAsync(primaryUri, {
          dialogTitle: cleanListingTitle(item.tajuk) || "Property Listing",
          mimeType: "image/jpeg",
          UTI: "public.jpeg",
        });
      } else {
        await Share.share({ message, title: cleanListingTitle(item.tajuk) });
      }
    } catch (e) {
        ...
        >
          ...
          >
            ...
            {shareModalListing?.tajuk ? (
              ...
              >
                {cleanListingTitle(shareModalListing.tajuk)}
              </Text>
            ) : (
              <View style={{ marginBottom: 12 }} />
            )}
            ...
            <View style={{ gap: 12 }}>
                ...
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                    <Text style={{ fontSize: 15, fontWeight: "700", color: themeColors.textPrimary }}>
                      {language === "BM"
                        ? `Kongsi Foto (${shareModalListing ? getListingImagesList(shareModalListing).length : 0} Foto)`
                        : `Share Photos (${shareModalListing ? getListingImagesList(shareModalListing).length : 0} Photos)`}
                    </Text>
                  </View>
              ...
              <TouchableOpacity
                activeOpacity={0.75}
                onPress={() => shareModalListing && handleCopyListingText(shareModalListing)}
                style={{
                  flexDirection: "row",
                  alignItems: "center",
                  padding: 16,
